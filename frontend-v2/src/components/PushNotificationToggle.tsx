...
/**
 * Ported from the old frontend's golf-only PushNotificationToggle
 * (2026-08-18) and generalized - one subscription now covers every push
 * source this app has (live golf scores, and the new odds-swing alerts
 * from scripts/detect_odds_swings.py), not just golf, so this lives in
 * the shared PageHeader instead of a single game's own page.
 */
  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      <button
        ...
        disabled={isPending}
        title="Alerts for live golf scores and large bookmaker odds swings on any player."
        className={`rounded-md border px-2.5 py-1.5 text-xs font-medium disabled:opacity-40 ${
          subscribed
            ...
            : "border-navy-700 text-navy-300 hover:border-sky-500 hover:text-white"
        }`}
      >
        {isPending ? "..." : subscribed ? "Alerts on" : "Enable alerts"}
      </button>
