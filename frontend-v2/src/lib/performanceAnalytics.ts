/**
 * Mary Performance Lab - reusable analytics over the prediction archive
 * (lib/predictionArchive.ts) and its evaluations. Pure functions only, no
 * Supabase calls - the page fetches rows, this module turns them into the
 * numbers the dashboard renders. Same "service, not embedded-in-JSX
 * logic" pattern as squadHealth.ts and recommendationScoring.ts.
 */
...
export type PredictionEvalRow = {
  ...
  // squad - null for hold predictions, where "applied" isn't a
  // meaningful question, and null until graded (see
  // prediction_evaluations.was_followed, computed by
  // scripts/evaluate_predictions.py against the real squad_transfers/
  // squad_captain_history logs, gameweek-scoped).
  ...
  evaluation: {
    ...
    // Real points swing of the choice actually made, when it differed
    // from Mary's recommendation (null if followed, or not yet graded).
    // Positive = the recommendation would have scored more than what the
    // user actually did; negative = the user's own choice outscored it.
    counterfactualPlayerId: number | null;
    counterfactualGain: number | null;
  } | null;
};
...
export type LifetimeSummary = {
  ...
  // Real cost/benefit of the recommendations NOT followed - sum of
  // counterfactualGain across every graded, not-followed transfer/captain
  // prediction. Positive = following Mary more would have scored more
  // overall; negative = the user's own judgement outscored her on
  // balance for the moves they chose not to take.
  unfollowedCount: number;
  totalCounterfactualGain: number | null;
};
...
export function computeLifetimeSummary(rows: PredictionEvalRow[]): LifetimeSummary {
  ...
  const successRate = successes.length > 0 ? successes.filter((r) => r.evaluation!.transferSuccess).length / successes.length : null;
  ...
  const captainSuccessRate =
    captainSuccesses.length > 0 ? captainSuccesses.filter((r) => r.evaluation!.captainSuccess).length / captainSuccesses.length : null;
  ...
  const unfollowed = rows.filter((r) => r.applied === false && r.evaluation?.counterfactualGain != null);
  const counterfactualGains = unfollowed.map((r) => r.evaluation!.counterfactualGain!);
  ...
  return {
    ...
    totalCounterfactualGain: counterfactualGains.length > 0 ? counterfactualGains.reduce((a, b) => a + b, 0) : null,
  };
}
 ...
 * group - those two concepts are transfer-specific.
 */
