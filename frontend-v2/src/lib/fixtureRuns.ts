 ...
 * other consumers (e.g. lib/fixtureSwing.ts) can reuse the same
 * per-gameweek numbers without re-deriving them - detectFixtureRuns
 ...
 * to scan (default "combined"). `options.minLength` overrides the minimum
 * run length (default MIN_RUN_LENGTH, 3).
 */
