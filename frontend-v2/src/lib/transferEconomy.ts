...
// activation. Extracted so the Ask Mary bundle search (which needs to know
// the cost of a hypothetical Nth transfer before it happens) and the real
// enforcement path can't drift apart.

export const TRANSFER_HIT_COST = -4;

// The real banking cap - Ask Mary's gameweek plan needs to actually
// simulate accrual forward across several gameweeks.
...
export function isWildcardActive(gameweek: number, wildcard1UsedGameweek: number | null, wildcard2UsedGameweek: number | null): boolean {
 ...
 * consumes one, the same way real bookkeeping does.
 */
...
// point-cost-per-extra-transfer mechanism exists like FanTeam's. No
// wildcard concept either.
 ...
 * netGain = expectedGain + cost calculation in askMaryEngine.ts so an
 * over-the-cap transfer is never chosen, without that search logic
 * needing a separate "is this even legal" branch of its own.
 */
