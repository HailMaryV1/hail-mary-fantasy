...
export default async function CloudFFPerformanceLabPage() {
  ...
  const header = (
    <div>
      ...
      <p className="mt-1 text-sm text-navy-300">Every recommendation Ask Mary has made for {squad?.name ?? "Cloud FF"}, measured against what actually happened.</p>
    </div>
  );
        ...
        <main className="mx-auto max-w-2xl">
          ...
          <div className="mt-8 rounded-xl border border-navy-700 bg-navy-900 p-6">
            <p className="text-sm text-navy-300">
              No predictions yet -{" "}
              <Link href="/cloudff/ask-mary" className="text-sky-400 hover:text-sky-300">
                visit Ask Mary
              </Link>{" "}
  ...
  const rows: PredictionEvalRow[] = predictions.map((p) => {
    ...
    return {
      ...
      // was_followed is only ever populated for transfer/hold predictions
      // right now - evaluate_predictions.py's captain-adherence path
      // keys off squad_captain_history, which Cloud FF's real per-
      // match-day captain (squad_match_day_captains) never writes to. A
      // real, known gap - captaincy predictions below will show
      // "Awaiting result" once graded but never a Followed/Not followed
      // badge until a squad_match_day_captains-aware adherence path
      // exists (a genuine follow-up, not part of this port).
  ...
  // (see lib/cloudffAskMaryEngine.ts) - each leg is its own predictions
