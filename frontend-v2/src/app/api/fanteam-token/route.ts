 ...
 * /fanteam/sync-setup, after the account owner has logged into FanTeam
 * themselves in their own browser (passing FanTeam's own reCAPTCHA the
 * normal way - this endpoint never attempts a FanTeam login itself, it
 * only relays a token a real human session already holds). Runs
 * cross-origin from fanteam.com's own page, so this needs its own CORS
 * handling and its own secret-based auth - there's no Hail Mary session
 * cookie available from that origin.
 *
 ...
 *
 * Also stores refreshToken (optional - encrypted_refresh_token) when the
 * bookmarklet found one - see open_authenticated_page's docstring in
 * provider_fanteam_scoutgg.py for why ftToken alone isn't enough.
 *
 * Ported from the old frontend project (2026-08-05) - this route only
 * ever existed there, and once hailmaryfantasysports.co.uk started
 * serving frontend-v2 instead, the bookmarklet had nowhere to send its
 * token (404), silently breaking re-authentication entirely.
 */

const FANTEAM_ORIGINS = ["https://fanteam.com", "https://www.fanteam.com"];
// FanTeam's real token lifetime is ~3h - a 15-minute safety margin means
// sync_fanteam always sees a token as expired slightly before FanTeam
// itself would reject it, never the other way around.
