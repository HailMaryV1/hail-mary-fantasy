...
// ±5pt threshold matches the retired algorithm-performance page's own
// "got it right" definition - kept identical so a resurrected number
// means the same thing it always did, not a fresh unvalidated cutoff.
...
export default function GolfScoresTable({ data }: { data: GolfScoreRow[] }) {
  ...
  const overrated = graded.filter((r) => r.pointsDifference! < -ACCURACY_THRESHOLD).length;
  const underrated = graded.filter((r) => r.pointsDifference! > ACCURACY_THRESHOLD).length;
  const withinThreshold = gradedCount - overrated - underrated;
  const accuracyPct = gradedCount > 0 ? (withinThreshold / gradedCount) * 100 : null;
      ...
      {gradedCount > 0 && (
        <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <div className="rounded-xl border border-navy-700 bg-navy-900 p-4">
            ...
            <p className="mt-1 text-xl font-semibold text-emerald-400">{accuracyPct?.toFixed(0)}%</p>
            <p className="mt-0.5 text-[11px] text-navy-500">within ±{ACCURACY_THRESHOLD}pts</p>
          </div>
          <div className="rounded-xl border border-navy-700 bg-navy-900 p-4">
