...
export default function GolfTeamBuilder({
  ...
  // (whatever pickBestMarket() chose for this tournament) - null when no
  // odds have been pasted at all, in which case no golfer will have a
                  ...
                  >
                    <span className="min-w-0 text-white">
                      <span className="block truncate" title={p.fullName}>
                        ...
                        {marketGapFlag === "value" && (
                          <span
                            title={`The bookies market says ${p.fullName} is ${(p.marketGap!.marketProbability * 100).toFixed(0)}% likely to finish ${marketLabel} - FanTeam's price only suggests ${(p.marketGap!.predictedProbability * 100).toFixed(0)}%.`}
                        ...
                        {marketGapFlag === "danger" && (
                          <span
                            title={`FanTeam's £${p.price.toFixed(1)}m price suggests a ${(p.marketGap!.predictedProbability * 100).toFixed(0)}% chance of a ${marketLabel} finish - the bookies market only gives ${p.fullName} ${(p.marketGap!.marketProbability * 100).toFixed(0)}%.`}
                          ...
                          <td colSpan={4} className="px-4 py-2 text-xs text-navy-400">
                            ...
                            {marketGapFlag === "value" && (
                              <span className="ml-3 font-semibold text-amber-400">
                                The bookies market says he&apos;s {(p.marketGap!.marketProbability * 100).toFixed(0)}% likely to finish {marketLabel} - FanTeam&apos;s price only suggests {(p.marketGap!.predictedProbability * 100).toFixed(0)}%.
                              </span>
                            )}
                            {marketGapFlag === "danger" && (
                              <span className="ml-3 font-semibold text-red-400">
                                FanTeam&apos;s price suggests a {(p.marketGap!.predictedProbability * 100).toFixed(0)}% chance of a {marketLabel} finish - the bookies market only gives him {(p.marketGap!.marketProbability * 100).toFixed(0)}%.
                              </span>
                            )}
                          </td>
                        </tr>
