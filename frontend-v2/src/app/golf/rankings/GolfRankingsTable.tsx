...
export type GolfRankingRow = {
  ...
  // Bookies-vs-price-predicted info (see golfValuePicks.ts) - null when
  // there's no odds for them. Badge classification (VALUE/DANGER/
  // neither) happens via classifyMarketGap(). Which market this is (top
  // 10/top 20/win/...) comes from the `market` prop below, not per-row -
  // one market applies to the whole table.
  marketGap: MarketGapInfo | null;
};

const MARKET_LABEL: Record<string, string> = { win: "win", top5: "top 5", top10: "top 10", top20: "top 20" };
...
export default function GolfRankingsTable({
  ...
  data: GolfRankingRow[];
  // Which market the marketGap badges/tooltips below are relative to
  // (whatever pickBestMarket() chose for this tournament) - null when
  // no odds have been pasted at all, in which case no row will have a
  // marketGap to begin with.
  market: string | null;
}) {
  ...
  const marketLabel = market ? (MARKET_LABEL[market] ?? market) : "finish";
                ...
                <Fragment key={row.gamePlayerId}>
                  ...
                  >
                    <td className="px-4 py-3 font-medium text-white">
                      ...
                      {marketGapFlag === "value" && (
                        <span
                          title={`The bookies market says ${row.fullName} is ${(row.marketGap!.marketProbability * 100).toFixed(0)}% likely to finish ${marketLabel} - FanTeam's price only suggests ${(row.marketGap!.predictedProbability * 100).toFixed(0)}%.`}
                      ...
                      {marketGapFlag === "danger" && (
                        <span
                          title={`FanTeam's £${row.price.toFixed(1)}m price suggests a ${(row.marketGap!.predictedProbability * 100).toFixed(0)}% chance of a ${marketLabel} finish - the bookies market only gives ${row.fullName} ${(row.marketGap!.marketProbability * 100).toFixed(0)}%.`}
                      ...
                      <td colSpan={COLUMNS.length} className="px-4 py-2 text-xs text-navy-400">
                        ...
                        {marketGapFlag === "value" && (
                          <span className="ml-3 font-semibold text-amber-400">
                            The bookies market says he&apos;s {(row.marketGap!.marketProbability * 100).toFixed(0)}% likely to finish {marketLabel} - FanTeam&apos;s price only suggests {(row.marketGap!.predictedProbability * 100).toFixed(0)}%.
                          </span>
                        )}
                        {marketGapFlag === "danger" && (
                          <span className="ml-3 font-semibold text-red-400">
                            FanTeam&apos;s price suggests a {(row.marketGap!.predictedProbability * 100).toFixed(0)}% chance of a {marketLabel} finish - the bookies market only gives him {(row.marketGap!.marketProbability * 100).toFixed(0)}%.
                          </span>
                        )}
                      </td>
                    </tr>
