...
export default async function GolfRankingsPage({
    ...
    return (
      <div className="min-h-screen bg-navy-950 px-4 py-6 sm:px-6">
        <div className="mx-auto max-w-3xl">
          ...
          <h1 className="mt-4 text-2xl font-semibold text-white">Hail Mary Golf Rankings</h1>
  ...
  if (tournament) {
    ...
    bestMarket = pickBestMarket((allOddsRows ?? []).map((o) => ({ golferId: o.golfer_id, market: o.market, impliedProbability: o.implied_probability })));
  ...
  return (
    <div className="min-h-screen bg-navy-950 px-4 py-6 sm:px-6">
      <div className="mx-auto max-w-4xl">
        <Link href="/golf/rankings" className="text-sm font-medium text-navy-400 hover:text-sky-400">
          ← All tournaments
        </Link>
        ...
        {tournament && rows.length > 0 && <GolfRankingsTable data={rows} market={bestMarket} />}
      </div>
    </div>
  );
}
