...
// EFL Fantasy never produces a "captain" kind prediction - no per-
// gameweek captain concept exists at all (see eflfantasyAskMaryEngine.ts's
// docstring, confirmed live: no "Set as captain" UI on the real site
// outside the Max Captain booster, which isn't archived as a prediction
// since nothing tracks whether it was triggered). Trimmed accordingly -
// unlike cloudff/performance-lab/page.tsx, no captain summary tiles or
// history branch.
export default async function EFLFantasyPerformanceLabPage() {
  ...
  const header = (
    <div>
      ...
      <p className="mt-1 text-sm text-navy-300">Every recommendation Ask Mary has made for {squad?.name ?? "EFL Fantasy"}, measured against what actually happened.</p>
    </div>
  );
  ...
  const { data: predictionsRaw } = await supabase
    .from("predictions")
    .select("id, squad_id, gameweek, algorithm_version_id, strategy, planning_horizon, kind, recommendation_type, rank, mary_move_score, confidence, risk, expected_gain, created_at, out_game_player_id, in_game_player_id")
    .eq("user_id", user.id)
        ...
        <main className="mx-auto max-w-2xl">
          ...
          <div className="mt-8 rounded-xl border border-navy-700 bg-navy-900 p-6">
            <p className="text-sm text-navy-300">
              No predictions yet -{" "}
              <Link href="/eflfantasy/ask-mary" className="text-sky-400 hover:text-sky-300">
                visit Ask Mary
              </Link>{" "}
  ...
  const { data: evaluationsRaw } = await supabase
    ...
    .select("prediction_id, actual_gain, prediction_error, transfer_success, error_attribution, was_followed, counterfactual_player_id, counterfactual_actual_points, counterfactual_gain")
  ...
  const referencedGamePlayerIds = Array.from(
    new Set([
      ...predictions.flatMap((p) => [p.out_game_player_id, p.in_game_player_id].filter((id): id is number => id != null)),
    ...
    return {
      ...
      evaluation: ev
        ? {
            ...
            captainActualPoints: null,
            viceCaptainActualPoints: null,
            captainSuccess: null,
  ...
  // A gameweek-plan step (or the club-pick recommendation) can bundle
  // more than one simultaneous transfer - each leg is its own
  // predictions row sharing (gameweek, planningHorizon, kind,
  // recommendationType, createdAt), grouped back into one history entry
  // here.
  const historyGroups = (() => {
      ...
      <main className="mx-auto max-w-5xl">
        ...
        <section className="mt-6">
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 sm:grid-cols-3 lg:grid-cols-4">
            ...
            <Tile label="Suggestions Followed" value={formatPct(summary.applicationRate)} sub="of transfer picks" />
            <Tile
                ...
                <div key={g.key} className="rounded-lg border border-navy-800 bg-navy-900 p-3">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      {g.allEvaluated ? (
                        <span className={`rounded-full px-2 py-0.5 text-[10px] font-medium ${(g.totalActualGain ?? 0) >= 0 ? "bg-emerald-950 text-emerald-400" : "bg-red-950 text-red-400"}`}>
                          {formatPts(g.totalActualGain)}
                        </span>
                      ) : (
