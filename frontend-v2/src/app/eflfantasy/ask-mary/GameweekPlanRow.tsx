...
import type { GameweekPlanStep, BundleTransfer } from "@/lib/eflfantasyAskMaryEngine";
...
/**
 * One step of Mary's sequential player gameweek plan - own copy (not
 * shared with the other games') per this app's per-game independent
 * identity rule. No price shown anywhere - EFL Fantasy has no budget at
 * all (see gameConfig.ts's hasBudget), unlike every other game's
 * equivalent row.
 */
export default function GameweekPlanRow({ step, squadId }: { step: GameweekPlanStep; squadId: number }) {
  ...
  const netGain = step.transfers.reduce((sum, t) => sum + t.pointsGain, 0);
  ...
  return (
    <div className="rounded-xl border border-navy-700 bg-navy-900">
      <button onClick={() => setExpanded((e) => !e)} className="flex w-full items-start justify-between gap-3 p-4 text-left hover:bg-navy-800/60">
      ...
      {expanded && (
        <div className="border-t border-navy-800 p-4">
          {step.hold ? (
            <p className="text-sm text-emerald-300">Nothing beats what you already have - holding is the right call.</p>
          ) : (
            <>
                  ...
                  <div key={`${t.outGamePlayerId}-${t.inGamePlayerId}`}>
                    ...
                    <div className="mt-1 flex flex-wrap items-center gap-2 text-sm">
                      ...
                      <span className="inline-flex items-center font-medium text-white">
                        {t.inName}
                        <FormPill status={t.inFormStatus ?? null} />
                      </span>
                    ...
                    {t.alternatives && t.alternatives.length > 0 && (
                      <p className="mt-1 text-xs text-sky-300">
                        Equally strong alternative: {t.alternatives[0].outName} → {t.alternatives[0].inName} (
                        {t.alternatives[0].pointsGain >= 0 ? "+" : ""}
                        {t.alternatives[0].pointsGain.toFixed(1)} pts)
                      </p>
                    )}
                    <button onClick={() => setExpandedLegIdx(expandedLegIdx === i ? null : i)} className="mt-1 text-xs font-medium text-sky-400 hover:text-sky-300">
              ...
              <div className="mt-3 rounded-lg border border-navy-800 bg-navy-950 p-3">
                ...
                <p className="mt-1 text-sm text-navy-200">{step.resultingSquadExpectedPoints.toFixed(1)} pts projected for GW{step.gameweek} (players only)</p>
              </div>
