-- Surface hail_mary_rating_basis (migration 0140) through both rating-
-- reading RPCs, so the frontend can show a real "Real Odds" / "Recent
-- Form" / "Model" badge next to every rating instead of leaving the
-- user to guess what backs it (2026-08-23 user request).
...
returns table(
    ...
    price numeric, competition text, hail_mary_score numeric, hail_mary_rating smallint,
    hail_mary_rating_basis text, goal_projected numeric,
...
language sql stable as $$
  with scored as (
    select distinct on (pr.game_player_id) pr.game_player_id, pr.hail_mary_score, pr.hail_mary_rating, pr.hail_mary_rating_basis, pr.inputs
    from projections pr
  ...
  base as (
    select
      ...
      s.hail_mary_rating_basis as hail_mary_rating_basis,
    ...
    left join lateral (
      ...
      from fixtures f2
      where s.inputs is not null and f2.id = (s.inputs #>> '{fixtures,0,fixture_id}')::bigint
    ) fx on true
...
drop function if exists get_top_rated_players(text, integer, integer);
...
returns table(
    ...
    team_id bigint, team_name text, hail_mary_rating smallint, hail_mary_rating_basis text, hail_mary_score numeric,
    opponent_team_name text, fixture_is_home boolean, fixture_kickoff_at timestamptz
)
language sql stable as $$
  with scored as (
  ...
  select position, rnk, game_player_id, full_name, team_id, team_name, hail_mary_rating, hail_mary_rating_basis, hail_mary_score,
