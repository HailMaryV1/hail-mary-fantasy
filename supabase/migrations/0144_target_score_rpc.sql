-- get_top_target_score_players: the Target Score equivalent of
-- get_top_rated_players (migration 0141), for the new horizon-aware
-- ranking (2026-08-23 user request). Mirrors that RPC's shape (dedup
-- latest projections row, position-partitioned row_number, same
-- fixture-provenance join for the nearest fixture badge) with one
-- deliberate difference: at horizon=1 the ranking stays driven by the
-- EXISTING hail_mary_rating (unchanged, byte-identical to
-- get_top_rated_players' own output) - the user's own words were "best
-- player for THIS gameweek... which i guess is the ratings we already
-- have". Only horizon > 1 actually ranks by the new target_score
-- composite. Both cases still return every target_scores sub-rating
-- column, since the "no more meaningless tier pill, show me a real
-- breakdown" request applies at every horizon, including 1.
--
-- The inner join to target_scores is what enforces the "no rubbish"
-- eligibility gate here - that table only ever has a row for a player
-- who already passed is_rating_eligible at this exact anchor gameweek
-- (see compute_target_scores.py), so a fixture-only player with no real
-- odds/form simply has no row to join to and is silently excluded,
-- with zero extra logic in this RPC.
