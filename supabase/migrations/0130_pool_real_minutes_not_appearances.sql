-- Real bug (2026-08-20 user report): "Appearances" always showed 0 on
-- the Fantasy Stats panel, for every player, in every game - confirmed
-- live against Bukayo Saka's real 2025/26 dreamteam row (296 total pts,
-- 11 goals, 12 assists) and two other real rows: none of their raw_stats
-- JSON blobs contain a key literally called "appearances" at all (the
-- keys vary wildly per provider/season - "tacklePts" vs "tackles" vs
-- nothing, etc.) - so real_stats' `raw_stats->>'appearances'` extraction
-- was silently defaulting to 0 on every single row, not just some.
--
-- Fix: game_player_stats already has a genuine, always-populated
-- `minutes_played` column (a real dedicated column, not a raw_stats key
-- that may or may not exist) - confirmed live (2225 real minutes for
-- that same Saka row). Swap the broken appearances extraction for that
-- instead of trying to patch per-provider raw_stats key names - every
-- other column/join is byte-identical to migration 0129.
-- CREATE OR REPLACE VIEW can change a column's underlying expression but
-- not its output name - rename first so the replace below is a no-op rename.
alter view game_player_pool rename column real_appearances to real_minutes_played;
...
-- search_game_player_pool selects gpp.real_appearances directly (its own
-- RETURNS TABLE column list, not a `select *`), so the rename above would
-- otherwise break this function's next real call outright. Postgres won't
-- let CREATE OR REPLACE change a function's return type, hence the drop -
-- same pattern migration 0118 already used for this same function.
...
returns table(
    ...
    price numeric, competition text, hail_mary_score numeric, goal_projected numeric,
