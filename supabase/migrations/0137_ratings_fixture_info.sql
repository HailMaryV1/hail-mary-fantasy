-- Surface each rated player's real fixture (opponent, home/away, kickoff)
-- on the Hail Mary Ratings page (2026-08-23 user request) - "so we know
-- the rating matches the gameweek matches the fixture". Sourced from
-- fixtures via the SAME fixture_id already stored in the projection's
-- own inputs.fixtures[0] (the exact fixture the score was computed
-- from) - not re-derived from game_fixture_gameweeks independently,
-- which could in principle disagree with what actually got scored.
-- Works uniformly for both real players (inputs.fixtures[0] has no
-- opponent_team_name field consistently for CLUB rows - EFL Fantasy's
-- club-score inputs, compute_club_scores(), never writes one - so this
-- resolves the opponent by joining fixtures/teams on fixture_id instead
-- of reading a name out of the JSON, which works for both shapes).
