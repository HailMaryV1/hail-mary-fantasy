-- Real player stats in the pool (2026-08-19 user request, comparing
-- against fantasy.efl.com's own player popup - "Recent Gameweeks" points,
-- appearances/clean sheets/tackles/clearances/blocks/interceptions/key
-- passes/shots on target). This data was already being scraped and
-- stored (game_player_stats.raw_stats jsonb - see import_eflfantasy.py)
-- but nothing in game_player_pool/search_game_player_pool ever surfaced
-- it - the pool was 100% projection-based, zero real stats reachable
-- from the frontend.
--
-- real_* columns come from game_player_stats at gameweek = 0 - the
-- rolling CURRENT-SEASON-TO-DATE snapshot every game's importer already
-- writes there (see compute_projections.py's CURRENT_SEASON docstring)
-- - "most recent by created_at" rather than a hardcoded season label so
-- this stays correct as each game's importer rolls its own season
-- forward, without this view needing a per-game season constant of its
-- own. Appearances/tackles/clearances/blocks/interceptions/key_passes/
-- shots_on_target only ever populated for EFL Fantasy today (the only
-- importer that writes those raw_stats keys) - null/0 elsewhere, same
-- "absence of data is never treated as a real value" convention this
-- view already documents for ownership_pct.
--
-- last_gw/last_gw_points answer "what did they score last week" -
-- sourced from player_gameweek_predictions.actual_points (populated by
-- capture_gameweek_actuals.py + attach_gameweek_results.py, run every
-- refresh cycle), the most recent gameweek with a real result attached -
-- NOT a duplicate of real_total_points (a season-to-date cumulative
-- total), a single gameweek's real result.
