-- Surfaces team_fixture_difficulty.clean_sheet_source (migration 0149)
-- through get_market_odds, so the Market Odds page can show an honest
-- "odds not posted yet" state instead of a number indistinguishable
-- from real bookmaker data (2026-08-26 user request: "We shouldnt use
-- the fallback - its tricking the eye. IF there is no odds then the
-- market page should say failed to pull odds").
drop function if exists get_market_odds(text, int);
