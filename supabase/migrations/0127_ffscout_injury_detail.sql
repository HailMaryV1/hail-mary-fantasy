...
-- injuries) - 2026-08-20 user request: "we can also add this page into
-- our injury news"..."I just want the data feeding into Ask Mary - the
-- player pools with player status... just like we have with the
-- predicted lineups and the little injury snippet. this is more
-- extensive." No new page - richer detail on the EXISTING FFScout status
-- pill (migration 0122/0123), same real Premier League scope.
--
-- New columns are nullable and additive - the existing team-news scraper
-- (scripts/capture_ffscout_player_status.py, source='ffscout') never
-- writes them; only the new scripts/capture_ffscout_injury_detail.py
-- (source='ffscout_injuries') does. Doesn't touch anything compute_
-- projections.py reads for scoring - status/start_probability keep their
-- exact existing meaning and blend weight.
alter table ffscout_player_status add column detail text;
alter table ffscout_player_status add column expected_return_date date;

-- game_player_pool/search_game_player_pool: the existing `ffscout`
-- lateral join picks the single freshest row per player for status/
-- start_probability (correct - a doubt % genuinely changes match to
-- match). detail/expected_return_date are different in kind - a real
-- ACL return date doesn't change hour to hour - so they're sourced from
-- the freshest row that actually HAS one, which may not be the same row
-- the status came from (e.g. today's team-news doubt-% capture has no
-- detail; the injuries-page capture from earlier in the week does).
-- Coalescing them into one lateral join would silently drop real detail
-- whenever the freshest row happens to be a bare status update.
...
returns table (
  ...
  ffscout_detail text,
  ffscout_expected_return_date date,
