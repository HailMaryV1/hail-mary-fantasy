...
// ---------------------------------------------------------------------
// Step 2: tournament odds. Moved here from the now-deleted /golf/odds
// page - per the user, odds are a one-time input done once during this
// wizard, never revisited once teams are set, so there's no need for a
// separately-reachable page. Logic unchanged from the original
// golf/odds/actions.ts.
// ---------------------------------------------------------------------
...
// frontend/src/app/squads/actions.ts's dispatchProviderSyncWorkflow
// already uses for "Sync Now" (same repo, same already-configured
// GITHUB_ACTIONS_TOKEN Vercel env var, no new secret needed). Fire-and-
// forget, matching that same precedent: no polling for completion, just
// tell the user it's started and link to where the result will show up.
// ---------------------------------------------------------------------
