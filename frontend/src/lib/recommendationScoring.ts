import { DEFAULT_SECURITY_SCORE } from "./playerStatus";
...
// same house style as swingOpportunity.ts's DEFAULT_SWING_WEIGHTS. Tunable
// by editing the numbers here only.
 ...
 * comparison rather than a league-wide one like swingOpportunity.ts.
 */
export function scoreMoveCandidates(
  candidates: MoveCandidateInput[],
  strategy: Strategy
): MoveScore[] {
  ...
  const rawEfficiency = candidates.map((c) =>
    c.priceDelta <= 0 ? c.expectedPointsGain + 1 : c.expectedPointsGain / Math.max(c.priceDelta, 0.5)
  );
  ...
  return candidates.map((c, i) => {
    ...
    const securityGain = normalize(
      c.incomingMinutesSecurity - c.outgoingMinutesSecurity,
      securityGainRange.min,
      securityGainRange.max
    );
    ...
    // matches this codebase's "skip or lower confidence, never fake"
    // rule for unavailable data.
    let confidence = 100;
    ...
    if (c.fixtureSwingDiff > 0.05) {
      reasons.push({
        code: "fixture_swing",
        text: `${c.inTeam}'s fixture outlook is improving relative to ${c.outTeam}'s.`,
      });
    }
    if (c.incomingMinutesSecurity - c.outgoingMinutesSecurity > 0.1) {
      reasons.push({
        code: "minutes_security",
        text: `${c.inName} has stronger expected minutes than ${c.outName}.`,
      });
    }
    if (c.priceDelta < 0) {
