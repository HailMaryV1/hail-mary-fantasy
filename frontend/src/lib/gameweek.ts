 ...
 * a different question from squads/actions.ts's getCurrentGameweek
 * (which answers "any fixture still upcoming," the right question for
 * free-transfer/-4pt bookkeeping, since you're still "in" a gameweek for
 * rules purposes until it's fully over). Here we want each gameweek's
 * OWN earliest kickoff specifically: the instant a gameweek's first ball
 * is kicked, planning shifts to the next one, even if that gameweek still
 * has fixtures left to play.
 */
