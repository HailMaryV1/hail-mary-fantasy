 ...
 * not as mediocre. Mirrors the "self-relative baseline" philosophy
 * already established in fixtureRuns.ts rather than inventing an
 * absolute points threshold nobody agreed on.
 */
...
export function assessSquadHealth(
  ...
  for (const pos of POSITIONS) {
    ...
    const avgRatio =
      posStarters.reduce((sum, p) => sum + qualityRatio(p.score, positionAverages[pos] ?? 0), 0) / posStarters.length;
    const avgSecurity =
      posStarters.reduce((sum, p) => sum + (LINEUP_SECURITY_SCORES[p.lineup ?? ""] ?? DEFAULT_SECURITY_SCORE), 0) /
      posStarters.length;
