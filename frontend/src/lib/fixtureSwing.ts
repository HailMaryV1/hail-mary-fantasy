 ...
 * ratio (computeTeamGameweekRatios, already proven by the Fixture ticker)
 * averaged over its next CURRENT_WINDOW gameweeks. "Upcoming" = whichever
 * good/bad run starts soonest, reusing that run's avgScore exactly as
 * fixtureRuns.ts already computes it - fixtures/page.tsx computes this
 * same number today but never renders it. swingValue is just their
 * difference. A team with no detected run (nothing crosses the >=1.15/
 * <=0.85 thresholds for 3+ gameweeks) is "stable", not an error.
 */
 ...
 * fixtures/game_fixture_gameweeks join fixtures/page.tsx already does.
 */
